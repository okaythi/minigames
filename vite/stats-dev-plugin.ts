          ...
          await withStore(async (store, memory) => {
            ...
            const games = {
              ...
              'fl-tron-3': {
                ...
                title: 'FL Tron 3.0',
                plays: 41,
                highscore: (playerRecord?.games['fl-tron-3']?.highscore && playerRecord.games['fl-tron-3'].highscore > 1000) ? playerRecord.games['fl-tron-3'].highscore : null,
                candy: playerRecord?.games['fl-tron-3']?.candy ?? 14,
                globalHighscore: (memory.games.get('fl-tron-3')?.highscore && (memory.games.get('fl-tron-3')?.highscore ?? 0) > 1000) ? memory.games.get('fl-tron-3')!.highscore : null,
                isRecordHolder: false,
                percentile: 'Top 25%',
                updatedAt: Date.now(),
              },
