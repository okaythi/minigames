...
function drawLoadout(ctx: CanvasRenderingContext2D, engine: PongEngine, fonts: PongFonts): void {
  ...
  for (const item of SHOP_ITEMS) {
    ...
    drawShopItem(ctx, y, item.name, `${COSTS[item.type]} C`, fonts)
  }
  ...
  for (let i = 0; i < engine.state.slots.length; i += 1) {
    ...
    if (slot !== null) {
      setFont(ctx, 800, 11, fonts.mono)
      ctx.fillStyle = PALETTE.blueDeep
      ctx.textAlign = 'center'
      ctx.fillText(slot[0]?.toUpperCase() ?? '', slotX + slotWidth / 2, 359)
    }
    slotX += slotWidth + gap
  }
...
function drawShopItem(ctx: CanvasRenderingContext2D, y: number, name: string, cost: string, fonts: PongFonts): void {
