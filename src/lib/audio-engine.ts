/**
 * Web Audio scheduler. Synthesizes procedural sound design with oscillators
 * and shaped noise bursts. Nothing is created until the first gesture.
 */
...
export type SfxCatalog<TSfx extends string> = Readonly<Record<TSfx, SoundDesign>>
...
export interface IAudioEngine<TSfx extends string = string> {
...
export class AudioEngine<TSfx extends string> implements IAudioEngine<TSfx> {
  ...
  public constructor(
    private readonly catalog: SfxCatalog<TSfx>,
  ...
  public play(name: TSfx): void {
    ...
    const design = this.catalog[name]
    if (design === undefined) {
      return
    }
